...
import { createEnrollmentWithAddress, createPayment, createTicket, createTicketType, createUser, createHotel, createRoomWithHotelId, createBooking, findBooking } from '../factories';
...
describe('PUT /booking/:bookingId', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 403 when room is full', async () => {
      ...
      const response = await server.put(`/booking/${createdNewBooking.id}`).set('Authorization', `Bearer ${token}`).send({roomId: createdRoom.id});
